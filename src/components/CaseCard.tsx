...
const STATUS_BADGE: Record<string, { label: string; bg: string; color: string }> = {
  ...
  dismissed: { label: "Dismissed",      bg: "bg-slate-50",   color: "text-slate-500" },
};
...
export default function CaseCard({ c }: { c: Case }) {
  ...
  return (
    <Link
      to={`/cases/${c.id}`}
      className="group bg-white rounded-2xl border border-slate-200 hover:border-slate-300 hover:shadow-lg hover:shadow-slate-100 transition-all overflow-hidden block"
    >
      <div className="flex">
        <div className="shrink-0 w-32 h-32 sm:w-40 sm:h-40 bg-slate-100">
          {c.image_url && (
            <img
              src={c.image_url}
              alt=""
        ...
        <div className="flex-1 min-w-0 p-4 sm:p-5 space-y-2">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="font-bold text-slate-900 group-hover:text-rose-600 transition-colors truncate">
                {c.trademark?.name ?? "Unknown IP"}
              </div>
            </div>
            <div className="shrink-0 text-right">
              <div className="text-2xl font-black text-slate-900 leading-none">{scorePct}%</div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {host && (
              <span className="text-[10px] font-semibold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full truncate max-w-[180px]">
                {host}
              </span>
            )}
          </div>
