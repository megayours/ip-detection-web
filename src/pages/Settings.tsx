...
// Reusable signer details that populate every takedown notice (rights-holder
// legal name + contact + authorized signatory). Platforms like Etsy require
// these fields, so the takedown composer flags them as missing until set.
...
function TakedownProfileSection() {
  ...
  async function save(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const { profile } = await updateTakedownProfile(form);
  ...
  return (
    <section className="space-y-3">
      <div>
        ...
        <p className="mt-1 text-sm text-stone-500">
          Used to fill in every takedown notice. Platforms require the rights-holder's
          name, contact details, and an authorized signatory.
        </p>
      </div>
